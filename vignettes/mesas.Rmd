...
vignette: >
  ...
  %\VignetteIndexEntry{Using mesas()}
...
Instalamos [Infoelectoral](https://hmeleiro.github.io/infoelectoral/) y cargamos las librerías necesarias.
```{r}
...
Descargamos los resultados a nivel de mesa de las elecciones municipales de mayo de 2015 y nos quedamos con los del municipio de Madrid.
```{r}
...
Descargamos el shapefile del seccionado censal del [INE](https://www.ine.es/prodyser/cartografia/seccionado_2016.zip) y filtramos las secciones del municipio de Madrid.
...
La función `mesas()` de [Infoelectoral](https://hmeleiro.github.io/infoelectoral/) descarga los resultados a nivel de mesa electoral, pero para representar un mapa es necesario tenerlos a nivel de sección censal. Así que agrupamos los datos. También filtro los cuatro partidos más votados.
...
Creamos una columna en el data frame de los resultados electorales con los códigos identificadores de cada seccion censal (`CUSEC`). Esta columna ya existe con el mismo nombre en el shapefile del INE y es la concatenación de los códigos de la provincia + municipio + distrito + sección censal
...
Para visualizar el mapa usamos la librería [tmap](https://github.com/mtennekes/tmap), que permite representar mapas en forma de pequeños múltiplos.
